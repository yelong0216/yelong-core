...
public class DefaultSqlModelResolver implements SqlModelResolver{
	...
	public ConditionSqlFragment resolveToCondition(SqlModel sqlModel, boolean isTableAlias) {
		...
		Map<String, Object> extendAttributeConditionMap = getExtendAttributeConditionMap(sqlModel.getExtendAttributes(), isTableAlias ?  tableAlias : null);
	...
	/**
	 * 列与运算符中的列添加别名。
	 * 1、添加别名仅针对与列名中不存在“.”字符的。
	 * 
	 * @param conditionOperatorMap 条件运算符映射
	 * @param tableAlias 表别名
	 * @return 列添加表别名后的条件运算符映射。这是一个新的map
	 */
	protected Map<String,String> conditionOperatorAddAlias(Map<String,String> conditionOperatorMap , String tableAlias){
	 ...
	 */
	protected Map<String,Object> getModelNonNullAttributeConditionMap(Modelable model , Class<? extends Modelable> modelClass , boolean isTableAlias){
		Map<String,Object> attributeMap = new HashMap<String, Object>();
	 ...
	 * @return 列（列名添加了别名）与值映射
	 */
	protected Map<String,Object> getExtendAttributeConditionMap(Map<String,Object> extendAttributes,@Nullable String tableAlias){
	...
	/**
	 * 将映射的属性和条件运算符结合转换为{@link Condition} 条件
	 * 默认的操作符为 {@link #DEFAULT_OPERATOR}
	 * 
	 ...
	 * @return 解析后的条件集合
	 */
	protected List<Condition> afterResolveToCondition(SqlModel sqlModel , boolean isTableAlias , List<Condition> conditions){
		return conditions;
	}
