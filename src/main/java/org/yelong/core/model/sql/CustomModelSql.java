...
/**
 * 自定义model 的 sql。
 * 定义的sql在{@link ModelSqlFragmentFactory}中进行使用
 * 定制在不同数据库方言使用不同的sql
 * 
 * 自定义的sql遵循以下规范：
 * 		1、同model、同方言、同操作类型在第二次添加时将会覆盖之前的
 * 
 * 	一般在Model中的静态块中进行加载。当在类静态代码进行注册时，在调用{@link #getModelSql(Class, OperationType, String)}时要注意该 modelClass 已经被初始化了。
...
public final class CustomModelSql implements Comparable<CustomModelSql>{
	...
	private static synchronized boolean register(Class<?> modelClass, OperationType operationType, String dialect, String sql) {
